  ...
  twilioClient.chat.request({
      ...
      axios.get(`https://api.mercadolibre.com/items/${productId}`)
          .then((response) => {
              ...
              const shipping = response.data.warranty;
              
              if (shipping) {
                  message = `A informação que possuímos sobre garantia é: ${shipping}`;
              } else {
                  message = "Infelizmente o produto não possui informações sobre garantia. Para maiores informações entre em contato diretamente com vendedor pelo canal (CANAL_A_SER_ESPECIFICADO)";
              }

  ...
  twilioClient.chat.request({
      ...
      axios.get(`https://api.mercadolibre.com/items/${productId}`)
          .then((response) => {
              ...
              if (price) {
                  message = `A informação que possuímos sobre o preço é: ${price}`;
              } else {
                  message = "Infelizmente o produto não possui informações sobre preço. Para maiores informações entre em contato diretamente com vendedor pelo canal (CANAL_A_SER_ESPECIFICADO)";
              }
